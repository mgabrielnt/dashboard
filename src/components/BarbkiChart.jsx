...
const BarbkiChart = ({ isDashboard = false, timeRange = { type: '24h' } }) => {
  ...
  useEffect(() => {
    if (timeRange && JSON.stringify(timeRange) !== JSON.stringify(chartTimeRange)) {
      console.log("BarbkiChart: TimeRange prop changed:", timeRange);
  ...
  const fetchAvailablePeriods = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/barbki/available-periods", {
  ...
  const fetchData = async (range = chartTimeRange) => {
    try {
      setLoading(true);
      console.log("BarbkiChart: Fetching data with range:", range);
      ...
      const apiUrl = params.toString() 
        ? `http://localhost:5000/api/barbki?${params.toString()}`
        : "http://localhost:5000/api/barbki";
      
      console.log("BarbkiChart: API request URL:", apiUrl);
  ...
  const processDataByMode = (rawData, mode) => {
    ...
    if (mode === 'monthly') {
      ...
      standardData.forEach(item => {
        ...
        if (!quarterData[key]) {
          quarterData[key] = { bulan: key, bki: 0, count: 0, year: item.year, quarter };
        }
        
        quarterData[key].bki += item.bki;
      ...
      return Object.values(quarterData).map(item => ({
        bulan: item.bulan,
        bki: item.count > 0 ? item.bki / item.count : 0, // average
        year: item.year,
      ...
      standardData.forEach(item => {
        ...
        if (!yearData[year]) {
          yearData[year] = { bulan: year.toString(), bki: 0, count: 0, year };
        }
        
        yearData[year].bki += item.bki;
  ...
  const handleDownloadChart = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `bki-chart-${displayMode}.svg`;
  ...
  if (!data || data.length === 0) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height={isDashboard ? "250px" : "300px"}>
        <Typography variant="h6" color={colors.grey[100]}>No data available for BKI chart</Typography>
      </Box>
    );
  }
      ...
      >
        <ResponsiveBar
          ...
          axisLeft={{
            ...
            tickPadding: 5,
            tickRotation: 0,
            legend: isDashboard ? undefined : "BKI",
            legendPosition: "middle",
            legendOffset: -40,
            format: (value) => 
              value >= 1000000000 
          ...
          barAriaLabel={function (e) {
            return `BKI for ${e.indexValue}: ${e.formattedValue}`;
          }}
          animate={true}
          motionStiffness={90}
          motionDamping={15}
      ...
      >
        ...
        >
          ...
          <Typography id="chart-info-modal-description" sx={{ mt: 2 }}>
            This chart displays the BKI (Badan Klasifikasi Indonesia) data over time. You can 
            view the data in monthly, quarterly, or yearly formats using the dropdown selector.
          </Typography>
          
          <Typography sx={{ mt: 2 }}>
            {displayMode === 'monthly' && "Each bar represents the BKI value for a specific month."}
            {displayMode === 'quarterly' && "Each bar represents the average BKI value for a quarter (3 months)."}
            {displayMode === 'yearly' && "Each bar represents the average BKI value for an entire year."}
