...
const BarsciChart = ({ isDashboard = false, timeRange = { type: '24h' } }) => {
  ...
  useEffect(() => {
    if (timeRange && JSON.stringify(timeRange) !== JSON.stringify(chartTimeRange)) {
      console.log("BarsciChart: TimeRange prop changed:", timeRange);
  ...
  const fetchAvailablePeriods = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/barsci/available-periods", {
  ...
  const fetchData = async (range = chartTimeRange) => {
    try {
      setLoading(true);
      console.log("BarsciChart: Fetching data with range:", range);
      ...
      const apiUrl = params.toString() 
        ? `http://localhost:5000/api/barsci?${params.toString()}`
        : "http://localhost:5000/api/barsci";
      
      console.log("BarsciChart: API request URL:", apiUrl);
      ...
      setError(error.message || "Failed to load SCI data");
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const processDataByMode = (rawData, mode) => {
    ...
    if (mode === 'monthly') {
      ...
      standardData.forEach(item => {
        ...
        if (!quarterData[key]) {
          quarterData[key] = { bulan: key, sci: 0, count: 0, year: item.year, quarter };
        }
        
        quarterData[key].sci += item.sci;
      ...
      return Object.values(quarterData).map(item => ({
        bulan: item.bulan,
        sci: item.count > 0 ? item.sci / item.count : 0, // average
        year: item.year,
      ...
      standardData.forEach(item => {
        ...
        if (!yearData[year]) {
          yearData[year] = { bulan: year.toString(), sci: 0, count: 0, year };
        }
        
        yearData[year].sci += item.sci;
  ...
  const handleDownloadChart = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `sci-chart-${displayMode}.svg`;
  ...
  if (!data || data.length === 0) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height={isDashboard ? "250px" : "300px"}>
        <Typography variant="h6" color={colors.grey[100]}>No data available for SCI chart</Typography>
      </Box>
    );
  }
      ...
      >
        <ResponsiveBar
          ...
          barAriaLabel={function (e) {
            return `SCI for ${e.indexValue}: ${e.formattedValue}`;
          }}
          animate={true}
          motionStiffness={90}
          motionDamping={15}
      ...
      >
        ...
        >
          ...
          <Typography id="chart-info-modal-description" sx={{ mt: 2 }}>
            This chart displays the SCI (Standard Chartered Indonesia) data over time. You can 
            view the data in monthly, quarterly, or yearly formats using the dropdown selector.
          </Typography>
          
          <Typography sx={{ mt: 2 }}>
            {displayMode === 'monthly' && "Each bar represents the SCI value for a specific month."}
            {displayMode === 'quarterly' && "Each bar represents the average SCI value for a quarter (3 months)."}
            {displayMode === 'yearly' && "Each bar represents the average SCI value for an entire year."}
