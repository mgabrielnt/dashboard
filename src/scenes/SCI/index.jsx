...
const FinancialStatements = () => {
  ...
  const fetchData = () => {
    ...
    // Create axios instance with proper headers for this specific request
    const token = localStorage.getItem("token");
    
    axios
      .get("http://localhost:5000/api/sci", { 
        headers: {
      ...
      .catch(error => {
        ...
        if (error.response && error.response.status === 500) {
          ...
          showDialog("error", "Sesi Anda telah berakhir. Silakan login kembali.");
  ...
  return (
    <Box m="20px">
      ...
      <Box 
        ...
        mb={2}
      >
        <Header title="LAPORAN KEUANGAN" subtitle="Analisis Data Keuangan Perusahaan" />
      ...
      {/* Loading indicator outside of filter section */}
      {loading && (
        <LinearProgress 
          sx={{ 
            mb: 2, 
