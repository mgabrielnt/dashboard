...
// Ambil semua data dari tabel si_monthly_tb
router.get("/", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM si_monthly_tb");
...
// Tambah data ke tabel si_monthly_tb
router.post("/", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO si_monthly_tb (__connect_topic, __connect_partition, __connect_offset, coa, value, entitas, year, posting_period, data_date, timestamp) 
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const checkId = await pool.query("SELECT id FROM si_monthly_tb WHERE id = $1", [id]);
    if (checkId.rowCount === 0) {
      return res.status(404).json({ error: "Data tidak ditemukan" });
    }

    const result = await pool.query(
      `UPDATE si_monthly_tb SET __connect_topic = $1, __connect_partition = $2, __connect_offset = $3, coa = $4, value = $5, entitas = $6, year = $7, posting_period = $8, data_date = $9, timestamp = $10 WHERE id = $11 RETURNING *`,
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const result = await pool.query("DELETE FROM si_monthly_tb WHERE id = $1 RETURNING *", [id]);
